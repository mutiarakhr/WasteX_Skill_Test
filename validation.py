...
def validate_future_dates(df, sheet_name, logs):
    ...
    for col in cols:
        ...
        for idx in df[dt > pd.Timestamp.today()].index:
            log_anomaly(logs, sheet_name, idx, col, "Type 5", df.loc[idx, col])
...
def cross_sheet_validation(df_prod, df_bag, df_app, df_bag_app):
    ...
    for idx, row in df_app.iterrows():
        if row.get("activity_id") not in valid:
            log("biochar_application", idx, "activity_id", "Type 7", "orphan", row.get("activity_id"))

    merged = df_app.merge(df_prod[["activity_id","biochar_amount_kg"]], on="activity_id", how="left")
    ...
    for _, r in merged.iterrows():
        if pd.notna(r["total_weight"]) and pd.notna(r["biochar_amount_kg"]):
            ...
            if diff > 0.05:
                log("biochar_application", r["idx"], "total_weight", "Type 8", "mismatch", r["total_weight"])

    dup = df_app[df_app["activity_id"].duplicated(keep=False)]

    for idx, row in dup.iterrows():
        log("biochar_application", idx, "activity_id", "Type 10", "duplicate usage", row["activity_id"])
